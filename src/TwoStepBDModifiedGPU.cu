...
#include "TwoStepBDModifiedGPU.cuh"
...
/*! \file TwoSteBDModifiedGPU.cu
    \brief Defines GPU kernel code for Brownian integration on the GPU. Used by TwoStepBDModifiedGPU.
*/
...
__global__ void gpu_brownian_step_one_kernel(Scalar4* d_pos,
    ...
    if (local_idx < nwork)
        ...
        if (d_noiseless_t)
            ...
            // else
            //     vel.z = 0;
            // OVERWRITE: Store the actual net velocity (Force + Noise) 
            // so FrictionPair can calculate drag correctly.
            // v = (F_net + F_random) / gamma
            Scalar mass = vel.w;
            Scalar sigma = fast::sqrt(T / mass);
            NormalDistribution<Scalar> normal(sigma);
            vel.x = (net_force.x) / gamma + normal(rng);
            vel.y = (net_force.y) / gamma + normal(rng);
            if (D > 2)
                vel.z = (net_force.z) / gamma + normal(rng);
            else
                vel.z = 0;
        ...
        if (aniso)
            ...
            if (gamma_r.x > 0 || gamma_r.y > 0 || gamma_r.z > 0)
                ...
                if (d_noiseless_r)
                    ...
                    // Calculate the net torque in the lab frame
                    // OVERWRITE: Calculate effective Angular Momentum from the step we just took.
                    
                    // 1. Calculate Angular Velocity in Lab Frame: w_lab = (Torque_net + Torque_rand) / gamma_r
                    // Note: 'bf_torque' was rotated to Lab Frame earlier in the kernel (line 186)
                    ...
                    p_vec.x = omega_body.x + NormalDistribution<Scalar>(fast::sqrt(T * I.x))(rng);
                    p_vec.y = omega_body.y + NormalDistribution<Scalar>(fast::sqrt(T * I.y))(rng);
                    p_vec.z = omega_body.z + NormalDistribution<Scalar>(fast::sqrt(T * I.y))(rng);
                    }
