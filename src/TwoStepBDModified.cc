...
*/
TwoStepBDModified::TwoStepBDModified(std::shared_ptr<SystemDefinition> sysdef,
    ...
    {
    m_exec_conf->msg->notice(5) << "Constructing TwoStepBDModified" << endl;
    }

TwoStepBDModified::~TwoStepBDModified()
    {
    m_exec_conf->msg->notice(5) << "Destroying TwoStepBDModified" << endl;
    }
...
*/
void TwoStepBDModified::integrateStepOne(uint64_t timestep)
    {
    ...
    for (unsigned int group_idx = 0; group_idx < group_size; group_idx++)
        ...
        // compute the bdmodified force (the extra factor of 3 is because <rx^2> is 1/3 in the uniform -1,1
        ...
        if (m_noiseless_t)
            ...
            //     h_vel.data[j].z = 0;
            // }
            // NEW CODE (Assign velocity based on the displacement formula)
            Scalar mass = h_vel.data[j].w;
            Scalar sigma = fast::sqrt(currentTemp / mass);
            NormalDistribution<Scalar> normal(sigma);
            h_vel.data[j].x = (h_net_force.data[j].x) / gamma + normal(rng);
            h_vel.data[j].y = (h_net_force.data[j].y) / gamma + normal(rng);
            if (D > 2)
                h_vel.data[j].z = (h_net_force.data[j].z) / gamma + normal(rng);
            else
                h_vel.data[j].z = 0;
            }
        ...
        if (m_aniso)
            ...
            if (gamma_r.x > 0 || gamma_r.y > 0 || gamma_r.z > 0)
                ...
                if (m_noiseless_r)
                    {
                    vec3<Scalar> omega_lab;
                    
                    // Safety check to avoid division by zero
                    omega_lab.x = (gamma_r.x > 0) ? ((t.x) / gamma_r.x)*I.x : 0;
                    omega_lab.y = (gamma_r.y > 0) ? ((t.y) / gamma_r.y)*I.y : 0;
                    omega_lab.z = (gamma_r.z > 0) ? ((t.z) / gamma_r.z)*I.z : 0;

                    vec3<Scalar> omega_body = rotate(conj(q), omega_lab);

                    p_vec.x = omega_body.x;
                    p_vec.y = omega_body.y;
                    p_vec.z = omega_body.z;
                    }
                else
                    {
                    ...
                    // p_vec.x = I.x * (t.x + bf_torque.x) / gamma_r.x;
                    // p_vec.y = I.y * (t.y + bf_torque.y) / gamma_r.y;
                    // p_vec.z = I.z * (t.z + bf_torque.z) / gamma_r.z;
                    
                    // OVERWRITE: Calculate effective Angular Momentum
    
                    // 1. Calculate Angular Velocity in Lab Frame: w_lab = (Torque_net + Torque_rand) / gamma_r
                    // Note: 'bf_torque' was rotated to Lab Frame earlier (line 166)
                    // and 't' is the net torque in Lab Frame.
                    vec3<Scalar> omega_lab;
                    ...
                    // 2. Rotate Angular Velocity to Body Frame
                    // w_body = conj(q) * w_lab
                    vec3<Scalar> omega_body = rotate(conj(q), omega_lab);

                    // 3. Calculate Angular Momentum: L_body = I * w_body
                    // We store this so FrictionPair can later do (L / I) to recover w_body.
                    p_vec.x = omega_body.x + NormalDistribution<Scalar>(fast::sqrt(currentTemp * I.x))(rng);
                    p_vec.y = omega_body.y + NormalDistribution<Scalar>(fast::sqrt(currentTemp * I.y))(rng);
                    p_vec.z = omega_body.z + NormalDistribution<Scalar>(fast::sqrt(currentTemp * I.z))(rng);
                    }
...
/*! @param timestep Current time step
 */
void TwoStepBDModified::integrateStepTwo(uint64_t timestep)
    {
