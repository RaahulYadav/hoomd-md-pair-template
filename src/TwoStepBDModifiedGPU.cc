...
*/
TwoStepBDModifiedGPU::TwoStepBDModifiedGPU(std::shared_ptr<SystemDefinition> sysdef,
                           std::shared_ptr<ParticleGroup> group,
                           std::shared_ptr<Variant> T,
                           bool noiseless_t,
                           bool noiseless_r)
    : TwoStepBDModified(sysdef, group, T, noiseless_t, noiseless_r)
    {
    if (!m_exec_conf->isCUDAEnabled())
        {
        throw std::runtime_error("Cannot create TwoStepBDModifiedGPU on a CPU device.");
        }

    m_tuner.reset(
        new Autotuner<1>({AutotunerBase::makeBlockSizeRange(m_exec_conf)}, m_exec_conf, "bdmodified"));
...
/*! \param timestep Current time step
   ...
   proper distribution.
*/
void TwoStepBDModifiedGPU::integrateStepOne(uint64_t timestep)
    {
...
*/
void TwoStepBDModifiedGPU::integrateStepTwo(uint64_t timestep)
    {
    // there is no step 2
    }
