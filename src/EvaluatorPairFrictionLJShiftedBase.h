...
#ifndef __PAIR_EVALUATOR_FRICTIONLJSHIFTEDBASE_H__
#define __PAIR_EVALUATOR_FRICTIONLJSHIFTEDBASE_H__
...
namespace md
    {
template<class Derived> class EvaluatorPairFrictionLJShiftedBase
    {
    public:
    ...
    struct param_type
        ...
        Scalar r_shift;   // Shift parameter r0
        ...
        */
        ...
        HOSTDEVICE param_type()
            : sigma_6(0), epsilon_x_4(0), gamma(0), kappa(0), pair_temp(0), r_shift(0)
            {
            }
        ...
        param_type(pybind11::dict v, bool managed)
            ...
            auto r_0(v["r_shift"].cast<Scalar>());
    ...
    */
    HOSTDEVICE EvaluatorPairFrictionLJShiftedBase(Scalar3& _dr,
                                                  Scalar3& _angvel_i,
                                                  Scalar3& _angvel_j,
                                                  Scalar3& _dv,
                                                  Scalar _dia_i,
        ...
        : dr(_dr), rcutsq(_rcutsq), angvel_i(_angvel_i), angvel_j(_angvel_j), dv(_dv),
          ...
          pair_temp(_params.pair_temp), r_shift(_params.r_shift)
        {
        }
    ...
    */
    HOSTDEVICE bool evaluate(Scalar3& force, Scalar& pair_eng, Scalar3& torque_i, Scalar3& torque_j)
        ...
        // Calculate r and shifted r
        Scalar r = fast::sqrt(rsq);
        Scalar r_shifted = r - r_shift;

        // If shifted distance is non-positive, treat as hard overlap / max force or zero?
        // For basic LJ, we typically just calculate it.
        // But physically r < r0 means we are "inside" the core shift.
        // Let's assume valid range is r > r_shift.
        if (r_shifted <= Scalar(0.0))
            return false; // Or ideally, handle overlap. But returning false avoids singularity.
        ...
        Scalar rinv = Scalar(1.0) / r_shifted;
        Scalar r2inv = Scalar(1.0) / r_shifted_sq;
        Scalar r6inv = r2inv * r2inv * r2inv;

        //! Calculation of the repulsive LJ-Force
        // force_divr_shifted = -(1/r') * dV/dr'
        Scalar force_divr_shifted
            = r2inv * r6inv * (Scalar(12.0) * lj1 * r6inv - Scalar(6.0) * lj2);

        // Force F = -(dV/dr) * r_hat
        // dV/dr = dV/dr' * (dr'/dr) = dV/dr' * 1
        // F = - (dV/dr') * r_hat
        //   = (Force_LJ_at_r_shifted) * r_hat
        // force_divr = F / r = (Force_LJ_at_r_shifted) / r
        // Force_LJ_at_r_shifted = force_divr_shifted * r_shifted
        // So force_divr = force_divr_shifted * (r_shifted / r)

        Scalar force_divr = force_divr_shifted * (r_shifted / r); // Scale force by geometry
        ...
        // rinv for e_ij calculation needs to be 1/r, NOT 1/r_shifted
        Scalar real_rinv = Scalar(1.0) / r;
        vec3<Scalar> e_ij = Scalar(-1.0) * rvec * real_rinv;
        ...
        if (TOLERANCE_DU < du)
            ...
            // w is usually the normal force magnitude (or related).
            // In original code: w = fast::sqrt(dot(f, f));
            // effectively just |F_normal|.
    ...
    Scalar r_shift;      //!< Radial shift parameter
...
#endif // __PAIR_EVALUATOR_FRICTIONLJSHIFTEDBASE_H__
